...
class MudElementReference {
    ...
    getClientRectFromParent(element) {
        if (element) {
            ...
            if (parent) return this.getBoundingClientRect(parent)
        }
    }
    getClientRectFromFirstChild(element) {
        if (element) {
            let child = element.children && element.children[0];
            if (child) return this.getBoundingClientRect(child)
        }
    }
    getBoundingClientRect(element) {
        if (element) {
            var rect = JSON.parse(JSON.stringify(element.getBoundingClientRect()));
            return rect.scrollY = window.scrollY || document.documentElement.scrollTop, rect.scrollX = window.scrollX || document.documentElement.scrollLeft, rect.windowHeight = window.innerHeight, rect.windowWidth = window.innerWidth, rect
        }
    }
    hasFixedAncestors(element) {
        for (; element && element !== document; element = element.parentNode)
            if (window.getComputedStyle(element).getPropertyValue("position") === "fixed") return !0;
        return !1
    }
    ...
    addEventListener(element, dotnet, event, callback, spec, stopPropagation) {
        ...
        return element.addEventListener(event, listener), this.eventListeners[++this.listenerId] = listener, this.listenerId
    }
    ...
    addDefaultPreventingHandler(element, eventName) {
        ...
        return element.addEventListener(eventName, listener, {
            passive: !1
        }), this.eventListeners[++this.listenerId] = listener, this.listenerId
    }
...
class MudJsEvent {
    ...
    attachHandlers(child) {
        ...
        for (var eventName of Object.getOwnPropertyNames(this._subscribedEvents)) this._subscribedEvents[eventName] && (this.logger("[MudBlazor | JsEvent] attaching event " + eventName, child), child.addEventListener(eventName, this.eventHandler));
...
class MudKeyInterceptor {
    ...
    setKeyOption(keyOption) {
        keyOption.key.length > 2 && keyOption.key.startsWith("/") && keyOption.key.endsWith("/") ? (keyOption.regex = new RegExp(keyOption.key.substring(1, keyOption.key.length - 1)), this._regexOptions.push(keyOption)) : this._keyOptions[keyOption.key.toLowerCase()] = keyOption;
    ...
    attachHandlers(child) {
        (this.logger("[MudBlazor | KeyInterceptor] attaching handlers ", {
            child
        }), this._observedChildren.indexOf(child) > -1) || (child.mudKeyInterceptor = this, child.addEventListener("keydown", this.onKeyDown), child.addEventListener("keyup", this.onKeyUp), this._observedChildren.push(child))
    }
    detachHandlers(child) {
    ...
    onKeyDown(args) {
        var self = this.mudKeyInterceptor,
            ...
            invoke, keyOptions, eventArgs;
        self.logger('[MudBlazor | KeyInterceptor] down "' + key + '"', args);
        invoke = !1;
        self._keyOptions.hasOwnProperty(key) && (keyOptions = self._keyOptions[key], self.logger('[MudBlazor | KeyInterceptor] options for "' + key + '"', keyOptions), self.processKeyDown(args, keyOptions), keyOptions.subscribeDown && (invoke = !0));
        for (const keyOptions of self._regexOptions) keyOptions.regex.test(key) && (self.logger('[MudBlazor | KeyInterceptor] regex options for "' + key + '"', keyOptions), self.processKeyDown(args, keyOptions), keyOptions.subscribeDown && (invoke = !0));
...
window.mudpopoverHelper = {
    ...
    placePopover: function(popoverNode, classSelector) {
        if (popoverNode && popoverNode.parentNode) {
            ...
            popoverContentNode.classList.contains("mud-popover-fixed") || (window.getComputedStyle(popoverNode).position == "fixed" ? popoverContentNode.style.position = "fixed" : (offsetX += window.scrollX, offsetY += window.scrollY));
...
class MudPopover {
    ...
    callback(id, mutationsList) {
        for (const mutation of mutationsList)
            if (mutation.type === "attributes") {
                ...
                if (mutation.attributeName == "class") target.classList.contains("mud-popover-overflow-flip-onopen") && target.classList.contains("mud-popover-open") == !1 && (target.mudPopoverFliped = null, target.removeAttribute("data-mudpopover-flip")), window.mudpopoverHelper.placePopoverByNode(target);
    ...
    initialize(containerClass, flipMargin) {
        ...
        mainContent.length != 0 && (flipMargin && (window.mudpopoverHelper.flipMargin = flipMargin), this.mainContainerClass = containerClass, mainContent[0].mudPopoverMark || (mainContent[0].mudPopoverMark = "mudded", this.contentObserver != null && (this.contentObserver.disconnect(), this.contentObserver = null), this.contentObserver = new ResizeObserver(() => {
...
class MudResizeObserver {
    constructor(dotNetRef, options) {
        ...
        this._resizeObserver = new ResizeObserver(entries => {
            var changes = [],
                target, affectedObservedElement, size;
            this.logger("[MudBlazor | ResizeObserver] changes detected");
            for (let entry of entries) target = entry.target, affectedObservedElement = observervedElements.find(x => x.element == target), affectedObservedElement && (size = entry.target.getBoundingClientRect(), affectedObservedElement.isInitialized == !0 ? changes.push({
    ...
    connect(elements, ids) {
        var result = [],
            i, newEntry;
        for (this.logger("[MudBlazor | ResizeObserver] Start observing elements..."), i = 0; i < elements.length; i++) newEntry = {
            element: elements[i],
            id: ids[i],
            isInitialized: !1
        }, this.logger("[MudBlazor | ResizeObserver] Start observing element:", {
            newEntry
        }), result.push(elements[i].getBoundingClientRect()), this._observervedElements.push(newEntry), this._resizeObserver.observe(elements[i]);
        return result
    }
    disconnect(elementId) {
        ...
        affectedObservedElement && (element = affectedObservedElement.element, this._resizeObserver.unobserve(element), this.logger("[MudBlazor | ResizeObserver] Element found. Ubobserving size changes of element", {
            element
        }), index = this._observervedElements.indexOf(affectedObservedElement), this._observervedElements.splice(index, 1))
    }
    cancelListener() {
...
class MudScrollSpy {
    ...
    scrollToSection(sectionId) {
        if (sectionId) {
            ...
            element && (this.scrollToSectionRequested = sectionId, element.scrollIntoView({
                behavior: "smooth",
                block: "center",
                inline: "start"
            }))
